...
public sealed class RemoveMediaFromExerciseCommandHandler : ICommandHandler<RemoveMediaFromExerciseCommand>
{
    private readonly IApplicationDbContext _db;
    private readonly ICurrentUserService _currentUser;
    private readonly IFileStorageService _storage;
    private readonly ILogger<RemoveMediaFromExerciseCommandHandler> _logger;

    public RemoveMediaFromExerciseCommandHandler(
        IApplicationDbContext db,
        ICurrentUserService currentUser,
        IFileStorageService storage,
        ILogger<RemoveMediaFromExerciseCommandHandler> logger)
    {
        _db = db;
    ...
    public async Task Handle(RemoveMediaFromExerciseCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!IsOwnerOrAdminOrGymManager(exercise, userId))
            throw new DomainException("Usuário não tem permissão para remover mídia.", ErrorCodes.ExerciseNotOwner);
        ...
        await _storage.DeleteAsync(media.FileId, cancellationToken);
        exercise.RemoveMedia(request.MediaId);

        await _db.SaveChangesAsync(cancellationToken);
        _logger.LogDebug("RemoveMediaFromExerciseHandler: SaveChangesAsync explícito concluído ExerciseId={ExerciseId}", exercise.Id);

        _logger.LogInformation(
            "ExerciseMediaRemoved ExerciseId={ExerciseId} MediaId={MediaId}",
            exercise.Id,
            request.MediaId);
    }
